...
$st = mysqli_prepare($conexion,
    "SELECT e.nombre FROM estudiantes e WHERE e.id = ? LIMIT 1");
...
$modulos = [
    [1,'Last Year',            'El pasado simple: verbos regulares e irregulares.',         '📅','modulo1_a2.html'],
    [2,'Bigger & Better',      'Comparativos y superlativos para describir y comparar.',     '📊','modulo2_a2.html'],
    [3,'Future Plans',         'Planes futuros con "going to" y "will".',                    '🔮','modulo3_a2.html'],
    [4,'How do you feel?',     'Emociones, estados y el vocabulario de salud.',              '😊','modulo4_a2.html'],
    [5,'Getting Around',       'Transporte, indicaciones y cómo moverte en inglés.',         '🚌','modulo5_a2.html'],
    [6,'At the Office',        'Trabajo, correos y vocabulario profesional básico.',         '💼','modulo6_a2.html'],
    [7,'Weekend Stories',      'Narrar eventos pasados con conectores (first, then...).',    '📖','modulo7_a2.html'],
    [8,'Making Plans',         'Invitaciones, sugerencias y rechazar/aceptar educadamente.','📅','modulo8_a2.html'],
];

// Resultados de quizzes A2
$quiz_status = [];
...
function modCard2(int $num, string $titulo, string $desc, string $icono, string $url, array $p, array $qs_all): string {
    $pct  = $p[$num]['pct']  ?? 0;
    $done = $p[$num]['done'] ?? false;
    $qs   = $qs_all[$num] ?? null;
    $color  = $done ? 'var(--success)' : ($pct > 0 ? '#ec4899' : 'var(--text-muted)');
    $label  = $done ? 'Completado' : ($pct > 0 ? 'En Progreso' : 'Sin comenzar');
    $bar    = $done ? '100%' : "$pct%";
    $barClr = 'linear-gradient(90deg,#ec4899,#f43f8e)';
    if ($qs && $qs['aprobado']) {
    ...
    $quizUrl = "/intep/cursoingles/quiz.php?nivel=A2&modulo={$num}";
    return <<<HTML
        <div style="display:flex;flex-direction:column;">
        <a href="{$url}" class="module-card">
            <div class="module-header">
                <span class="module-num" style="color:#f9a8d4;background:rgba(236,72,153,0.1);">Módulo {$num}</span>
                <span>{$icono}</span>
            </div>
            <h3>{$titulo}</h3>
            <p style="color:var(--text-muted);font-size:0.9rem;margin-top:10px;">{$desc}</p>
            <div class="module-progress"><div class="progress-bar" style="width:{$bar};background:{$barClr};"></div></div>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 4px 0;">
            {$quizBadge}
            <a href="{$quizUrl}" style="font-size:0.8rem;color:#f9a8d4;text-decoration:none;font-weight:600;padding:4px 10px;border-radius:8px;border:1px solid rgba(236,72,153,0.35);transition:background 0.2s;" onmouseover="this.style.background='rgba(236,72,153,0.15)'" onmouseout="this.style.background='transparent'">Examen →</a>
        </div>
        </div>
    HTML;
}
...
<head>
    ...
    <title>Dashboard A2 | INTEP Inglés</title>
    ...
    <style>
        ...
        .profile-avatar{width:50px;height:50px;background:#ec4899;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.4rem;font-weight:bold;font-family:'Outfit',sans-serif;overflow:hidden;flex-shrink:0;}
        ...
        .module-card:hover{transform:translateY(-5px);box-shadow:0 10px 25px rgba(0,0,0,0.3);border-color:#f9a8d4;background:rgba(255,255,255,0.1);}
        .module-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;}
        .module-num{padding:6px 12px;border-radius:10px;font-weight:600;font-size:0.9rem;}
        .module-progress{width:100%;background:rgba(255,255,255,0.1);height:8px;border-radius:10px;margin-top:20px;overflow:hidden;}
        .progress-bar{height:100%;background:linear-gradient(90deg,#ec4899,#f43f8e);border-radius:10px;transition:width 0.8s ease;}
        .nav-link{color:#cbd5e1;text-decoration:none;padding:12px;display:block;transition:color 0.3s;border-radius:8px;}
        .nav-link:hover{color:white;background:rgba(255,255,255,0.05);}
        .nav-link.active{color:#f9a8d4;font-weight:bold;}
...
<div class="dashboard-layout">
    <aside class="sidebar">
        ...
        <div class="profile-widget">
            ...
            <div>
                ...
                <p style="font-size:0.8rem;color:#cbd5e1;margin:4px 0 0;">Nivel: <?= htmlspecialchars($nivel_actual) ?> · <span style="background:rgba(236,72,153,0.2);border:1px solid rgba(236,72,153,0.4);color:#f9a8d4;padding:2px 8px;border-radius:10px;font-size:0.75rem;"><?= $xp_total ?> XP</span></p>
            </div>
        </div>
        <nav style="display:flex;flex-direction:column;gap:6px;">
            <div style="background:rgba(255,255,255,0.1);border-radius:10px;padding:5px;">
                ...
                <div style="display:flex;flex-direction:column;padding-left:20px;gap:4px;margin-bottom:8px;">
                    ...
                    <a href="/intep/cursoingles/dashboard_a2.php" class="nav-link active" style="font-size:0.9rem;">🔸 A2 Elementary</a>
    ...
    <main class="main-content">
        <h1>Tu Plan de <span style="background:linear-gradient(to right,#ec4899,#f43f8e);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Estudios A2</span></h1>
        <p style="color:var(--text-muted);font-size:1.1rem;">Nivel Elementary. Expande tu vocabulario y domina el pasado.</p>
        <div class="modules-grid">
            <?php foreach ($modulos as [$num,$titulo,$desc,$icono,$url]):
                echo modCard2($num,$titulo,$desc,$icono,$url,$progreso,$quiz_status);
            endforeach; ?>
        </div>

        <!-- Quiz General A2 -->
        <div style="margin-top:3rem;padding:2rem;border-radius:20px;border:2px solid <?= $quiz_general_disponible ? '#ec4899' : 'rgba(255,255,255,0.1)' ?>;background:<?= $quiz_general_disponible ? 'rgba(236,72,153,0.08)' : 'rgba(255,255,255,0.03)' ?>;">
            <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
                <div style="font-size:2.5rem;"><?= $quiz_general_aprobado ? '🏆' : ($quiz_general_disponible ? '📋' : '🔒') ?></div>
                <div style="flex:1;">
                    <h3 style="margin:0 0 4px;color:<?= $quiz_general_disponible ? 'white' : '#64748b' ?>;">Quiz General A2</h3>
                    <p style="margin:0;font-size:0.9rem;color:<?= $quiz_general_disponible ? '#94a3b8' : '#475569' ?>;">
                        <?php if ($quiz_general_aprobado): ?>
                            ✅ Aprobado con <?= $quiz_status[0]['score'] ?>% · ¡Nivel A2 dominado!
                        <?php elseif ($quiz_general_disponible): ?>
                            ¡Todos los módulos completados! Pon a prueba todo tu inglés A2.
                        <?php else: ?>
                ...
                <?php if ($quiz_general_disponible): ?>
                    <a href="/intep/cursoingles/quiz.php?nivel=A2&modulo=0" style="padding:12px 28px;border-radius:12px;background:<?= $quiz_general_aprobado ? 'rgba(16,185,129,0.2)' : 'linear-gradient(135deg,#ec4899,#f43f8e)' ?>;color:<?= $quiz_general_aprobado ? '#10b981' : 'white' ?>;font-weight:700;text-decoration:none;border:<?= $quiz_general_aprobado ? '1px solid #10b981' : 'none' ?>;">
