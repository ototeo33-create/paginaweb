...
if (!$input || !isset($input['modulo_id']) || !isset($input['asistencias']) || !is_array($input['asistencias'])) {
...
foreach ($input['asistencias'] as $a) {
    ...
    $check = mysqli_prepare($conexion, "SELECT id FROM asistencia WHERE estudiante_id = ? AND modulo_id = ?");
    ...
    if (mysqli_num_rows($existe) > 0) {
        ...
        $stmt = mysqli_prepare($conexion, "INSERT INTO asistencia (estudiante_id, modulo_id, total_clases, total_asistencias, total_inasistencias, porcentaje_asistencia) VALUES (?,?,?,?,?,?)");
        mysqli_stmt_bind_param($stmt, 'iiiiid', $estudiante_id, $modulo_id, $total_clases, $total_asistencias, $total_inasistencias, $porcentaje);
    }
